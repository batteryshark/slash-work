...
function strongestStatus(statuses) {
  return statuses.sort((left, right) => STATUS_PRIORITY[right] - STATUS_PRIORITY[left])[0] ?? null;
}
...
async function canInitializeProject(pathname) {
  try {
    await lstat(join(pathname, DATA_DIRECTORY));
    return false;
  } catch (error) {
...
export async function listFiles(workspace, { scopePath = ".", path = "." } = {}) {
  ...
  const visibleChildren = children.filter((child) =>
    !IGNORED_FILES.has(child.name)
    && !IGNORED_DIRECTORIES.has(child.name)
    && !(child.isDirectory() && child.name.endsWith(".egg-info")),
  );
  const entries = await Promise.all(visibleChildren.map(async (child) => {
    ...
    return {
      ...
      canInitializeProject: kind === "directory"
        ? await canInitializeProject(join(directory.pathname, child.name))
        : false,
