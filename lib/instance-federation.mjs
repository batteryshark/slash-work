...
function credentialError(error, action) {
  return new WorkspaceError(`The operating system credential store could not ${action} the remote Work access key.`, {
...
async function credentialGet(store, peerId) {
  try {
    return await store.get(peerId);
  } catch (error) {
    throw credentialError(error, "read");
  }
}

async function credentialSet(store, peerId, token) {
  try {
    await store.set(peerId, token);
  } catch (error) {
    throw credentialError(error, "save");
  }
}

async function credentialDelete(store, peerId) {
  try {
...
export class FederationManager {
  ...
  async addPeer({ baseUrl, accessKey }) {
    ...
    await credentialSet(this.credentialStore, manifest.instance.id, token);
  ...
  async removePeer(id) {
    ...
    await credentialDelete(this.credentialStore, id);
  ...
  async refreshPeer(peer) {
    try {
      const token = (await credentialGet(this.credentialStore, peer.id))?.trim();
  ...
  async peerToken(peerId) {
    const token = (await credentialGet(this.credentialStore, peerId))?.trim();
