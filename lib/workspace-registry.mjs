...
export async function registerWorkspace(rootPath, { force = false, idFloor = null, registryPath = workspaceRegistryPath() } = {}) {
  let canonical = null;
  try {
  ...
  const workspace = await initializeWorkspace(resolve(rootPath), { force, idFloor });
  const registry = await readRegistry(registryPath);
