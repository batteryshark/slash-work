...
export async function updateProjectProfile(workspace, projectPath, input, projects = null) {
  ...
  if (!hasName && !hasDescription && !hasView) {
    throw new WorkspaceError("Provide a project name, description, or view to update.", { code: "invalid_input" });
  }
  ...
  await atomicWrite(markerPath, `${JSON.stringify({ ...marker, name, description, view, updatedAt: new Date().toISOString() }, null, 2)}\n`);
...
function issueFromRecord(record, pathname) {
  ...
  return {
    ...
    scopePath: metadata.scopePath ?? ".",
    projectPath: metadata.projectPath ?? null,
    // Tolerant read: records written before the delegated boolean carried an
    // agents name list; any non-empty list means the item was handed off.
    delegated: metadata.delegated === true || (Array.isArray(metadata.agents) && metadata.agents.length > 0),
...
function taskFromRecord(record, pathname) {
  ...
  const task = {
    ...
    // estimate, and an agents list. Those keys are ignored here and dropped on
    // the next write; a non-empty agents list reads as delegated.
    delegated: metadata.delegated === true || (Array.isArray(metadata.agents) && metadata.agents.length > 0),
    // ponytail: iOS compat — the shipped app decodes a non-optional `agents`
    // list on tasks, so the payload keeps a constant empty list. The iOS pass
    // removes this once Models.swift drops the field. Never persisted:
    // writeTask lists frontmatter keys explicitly.
    agents: [],
