...
const RECORD_ID_PATTERN = /^(capture|decision|idea|issue|note)_[a-z0-9][a-z0-9_-]{7,80}$/;
...
async function ensureRecordStorage(root, dataPath) {
  ...
  const issuesPath = await ensureSafeDirectory(root, join(dataPath, "issues"));
  ...
  return { dataPath, capturesPath, decisionsPath, ideasPath, issuesPath, notesPath, tasksPath };
}
...
async function discoverProjectsFromRoot(root, { maxDepth, maxDirectories, concurrency }) {
  const projects = [];
  ...
  while (cursor < queue.length) {
    ...
    const batch = queue.slice(cursor, Math.min(queue.length, cursor + concurrency, maxDirectories));
    cursor += batch.length;
...
export async function discoverProjects(
  rootPath,
  {
    ...
    maxDirectories = DEFAULT_PROJECT_SCAN_DIRECTORY_LIMIT,
    concurrency = DEFAULT_PROJECT_SCAN_CONCURRENCY,
  } = {},
) {
  if (maxDepth !== Number.POSITIVE_INFINITY && (!Number.isInteger(maxDepth) || maxDepth < 0)) {
    throw new WorkspaceError("maxDepth must be a non-negative integer or Infinity.", { code: "invalid_discovery_limit" });
  }
  if (!Number.isInteger(maxDirectories) || maxDirectories < 1) {
    throw new WorkspaceError("maxDirectories must be a positive integer.", { code: "invalid_discovery_limit" });
  }
  if (!Number.isInteger(concurrency) || concurrency < 1 || concurrency > 64) {
    throw new WorkspaceError("concurrency must be an integer between 1 and 64.", { code: "invalid_discovery_limit" });
  }

  const root = await canonicalDirectory(rootPath);
  const key = `${root}\0${maxDepth}\0${maxDirectories}\0${concurrency}`;
  const active = projectDiscoveryFlights.get(key);
  if (active) return active;

  const flight = discoverProjectsFromRoot(root, { maxDepth, maxDirectories, concurrency })
    .finally(() => {
...
export async function updateProjectProfile(workspace, projectPath, input, projects = null) {
  ...
  if (!hasName && !hasDescription) {
    throw new WorkspaceError("Provide a project name or description to update.", { code: "invalid_input" });
  }
  ...
  const description = hasDescription
    ...
    : marker.description;
  await atomicWrite(markerPath, `${JSON.stringify({ ...marker, name, description, updatedAt: new Date().toISOString() }, null, 2)}\n`);
...
export async function migrateProjectLocalStorage(workspace) {
  ...
  for (const [directoryField, metadataField] of [["capturesPath", "projectPath"], ["decisionsPath", "projectPath"], ["ideasPath", "projectPath"], ["issuesPath", "projectPath"], ["notesPath", "projectPath"], ["tasksPath", "project_path"]]) {
...
function noteFromRecord(record, pathname) {
  ...
  return {
    ...
    projectPath: metadata.projectPath ?? null,
    createdBy,
    // ponytail: compat shim — iOS decodes agentIntent as non-optional. Legacy
    // default emitted in responses only; noteMarkdown never writes it to disk.
    // Remove in the iOS pass together with Models.swift/APIClient.swift cleanup.
...
export async function listCaptures(workspace, projects = null) {
  const captures = [];
  for (const { pathname, projectPath } of await listStoredMarkdown(workspace, "capturesPath", projects)) {
...
export async function createNote(workspace, input, projects = null, { createdBy = { kind: "human", name: null } } = {}) {
  ...
  const note = {
    ...
    projectPath,
    createdBy,
    agentIntent: "reference_only", // ponytail: iOS compat shim, response-only (see noteFromRecord)
    createdAt: now,
    updatedAt: now,
  };
...
export async function deleteNote(workspace, id, { agentName = null } = {}) {
  ...
  if (!located) throw new WorkspaceError("Note not found.", { code: "not_found", status: 404 });
  if (agentName) {
    const note = noteFromRecord(parseMarkdownRecord(await readFile(located.pathname, "utf8"), located.pathname), located.pathname);
    assertAgentOwnsNote(note, agentName);
  }
  try {
...
function assertIssueMarkdown(value, field, { max = MAX_TEXT_LENGTH } = {}) {
  if (typeof value !== "string") {
    throw new WorkspaceError(`${field} must be a string.`, { code: "invalid_input" });
  }
  if (!value.trim()) {
...
function validIssueActor(value) {
  ...
  if (
    ...
    return { kind: "agent", name: value.name.trim() };
  }
  return null;
}
...
async function writeIssue(issue, priorMetadata = {}, sourcePath = null, workspace) {
  const storage = await targetRecordStore(workspace, issue.projectPath);
  const pathname = sourcePath
    ? await relocateRecord(sourcePath, storage.issuesPath)
    : join(storage.issuesPath, `${issue.id}.md`);
  await atomicWrite(
    pathname,
    markdownRecord(
      {
...
async function readIssueRecord(workspace, id) {
  ...
  const located = await findStoredRecord(workspace, "issuesPath", `${id}.md`);
  if (!located) throw new WorkspaceError("Issue not found.", { code: "not_found", status: 404 });
  const record = parseMarkdownRecord(await readFile(located.pathname, "utf8"), located.pathname);
  const issue = issueFromRecord(record, located.pathname);
  if (located.projectPath) {
    issue.projectPath = located.projectPath;
    if (issue.scopePath === "." || !issue.scopePath.startsWith(`${located.projectPath}/`)) {
      issue.scopePath = located.projectPath;
    }
  }
  return { issue, record, pathname: located.pathname };
}

export async function listIssues(workspace, projects = null) {
  const issues = [];
  for (const { pathname, projectPath } of await listStoredMarkdown(workspace, "issuesPath", projects)) {
    const issue = issueFromRecord(parseMarkdownRecord(await readFile(pathname, "utf8"), pathname), pathname);
    if (projectPath) {
...
export async function createIssue(workspace, input, projects = null, { agentName = null } = {}) {
  const knownProjects = projects ?? (await discoverProjects(workspace.root));
  const body = assertIssueMarkdown(input?.body, "body");
  const title = input?.title == null
...
export async function updateIssue(workspace, id, input) {
  const { issue, record, pathname } = await readIssueRecord(workspace, id);
  const changed = [];
  if (input?.agents != null) { issue.agents = stringArray(input.agents, "agents"); changed.push("agents"); }
...
export async function updateIssueState(workspace, id, input, { agentName = null } = {}) {
  ...
  if (agentName) {
    ...
    const resolutionSummary = state === "resolved"
      ? assertIssueMarkdown(input?.resolutionSummary, "resolutionSummary")
      : null;
...
function ideaFromRecord(record, pathname) {
  ...
  return {
    ...
    // ponytail: compat shim — iOS decodes agentIntent as non-optional. Legacy
    // default emitted in responses only; writeIdea never writes it to disk.
    // Remove in the iOS pass together with Models.swift/APIClient.swift cleanup.
...
async function writeIdea(workspace, idea, priorMetadata = {}, sourcePath = null) {
  ...
  // Tolerant migration: drop the removed agentIntent field from legacy records
  // on the next write instead of failing or preserving it.
  const { agentIntent: _removedAgentIntent, ...retainedMetadata } = priorMetadata;
  await atomicWrite(
    pathname,
    markdownRecord(
      {
...
export async function listIdeas(workspace, projects = null) {
  const ideas = [];
...
export async function createIdea(workspace, input, projects = null) {
  ...
  const idea = {
    ...
    revisitAt: null,
    agentIntent: "consideration_only", // ponytail: iOS compat shim, response-only (see ideaFromRecord)
    history: [],
    createdAt: now,
    updatedAt: now,
    sections,
...
export async function listDecisions(workspace, projects = null) {
  const decisions = [];
  for (const { pathname, projectPath } of await listStoredMarkdown(workspace, "decisionsPath", projects)) {
...
export async function listTasks(workspace, projects = null) {
  const tasks = [];
  for (const { pathname, projectPath } of await listStoredMarkdown(workspace, "tasksPath", projects)) {
...
export async function workspaceSnapshot(workspace) {
  const projects = await discoverProjects(workspace.root);
  const [captures, decisions, ideas, issues, notes, tasks] = await Promise.all([
    listCaptures(workspace, projects),
    listDecisions(workspace, projects),
    listIdeas(workspace, projects),
