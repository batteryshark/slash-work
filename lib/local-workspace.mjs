...
async function ensureRecordStorage(root, dataPath) {
  ...
  return { dataPath, capturesPath, decisionsPath, ideasPath: join(dataPath, "ideas"), issuesPath, notesPath, tasksPath };
}
...
export async function migrateProjectLocalStorage(workspace) {
  ...
  for (const [directoryField, metadataField] of [["capturesPath", "projectPath"], ["decisionsPath", "projectPath"], ["issuesPath", "projectPath"], ["notesPath", "projectPath"], ["tasksPath", "project_path"]]) {
...
export async function workspaceSnapshot(workspace) {
  ...
  return {
    ...
    captures,
    decisions,
    // ponytail: iOS compat shim — Models.swift decodes `ideas` as non-optional,
    // so the snapshot keeps an empty constant until the iOS pass removes the
    // Ideas tab together with WorkIdea in Models.swift/IdeasView.swift.
    ideas: [],
    issues,
    notes,
    tasks,
