...
const RECORD_ID_PATTERN = /^(capture|decision|note)_[a-z0-9][a-z0-9_-]{7,80}$/;
...
async function ensureRecordStorage(root, dataPath) {
  ...
  return { dataPath, capturesPath, decisionsPath, notesPath, tasksPath };
}
...
export async function migrateProjectLocalStorage(workspace) {
  ...
  for (const [directoryField, metadataField] of [["capturesPath", "projectPath"], ["decisionsPath", "projectPath"], ["notesPath", "projectPath"], ["tasksPath", "project_path"]]) {
...
export async function listNotes(workspace) {
  ...
  for (const { pathname, projectPath } of await listStoredMarkdown(workspace, "notesPath")) {
    const note = noteFromRecord(parseMarkdownRecord(await readFile(pathname, "utf8"), pathname), pathname);
...
export async function workspaceSnapshot(workspace) {
  const projects = await discoverProjects(workspace.root);
  const [captures, decisions, notes, tasks] = await Promise.all([
