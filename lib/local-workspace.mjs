...
export async function updateProjectDescription(workspace, projectPath, input, projects = null) {
  ...
  await atomicWrite(markerPath, `${JSON.stringify({ ...marker, description, updatedAt: new Date().toISOString() }, null, 2)}\n`);
  const updated = (await discoverProjects(workspace.root)).find((project) => project.path === validatedPath);
  if (!updated) throw new WorkspaceError("Project could not be rediscovered after updating its description.", { code: "project_not_found", status: 409 });
  return updated;
}
