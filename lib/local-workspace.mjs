...
export async function createNote(workspace, input, projects = null, { createdBy = { kind: "human", name: null } } = {}) {
  ...
  note.text = withAttachmentRefs(note.text, note.id, await writeAttachments(workspace, note.id, input?.attachments));
...
export async function updateNote(workspace, id, input, { agentName = null } = {}) {
  ...
  updated.text = withAttachmentRefs(updated.text, id, await writeAttachments(workspace, id, input?.attachments));
...
// and the record's markdown references it relatively (../attachments/…), so
// any markdown viewer renders it and any local agent can open it straight
// from the reference. Remote clients fetch the same path over the API.
...
async function writeAttachments(workspace, recordId, list) {
  if (list == null) return [];
  ...
  const directory = attachmentsDirectory(workspace, recordId);
...
export async function readRecordAttachment(workspace, recordId, name) {
  ...
  try {
    const bytes = await readFile(join(attachmentsDirectory(workspace, recordId), name));
...
export async function createIssue(workspace, input, projects = null, { agentName = null } = {}) {
  ...
  issue.body = withAttachmentRefs(issue.body, issue.id, await writeAttachments(workspace, issue.id, input?.attachments));
...
export async function replyToIssue(workspace, id, input, { agentName = null } = {}) {
  ...
  appendIssueMessage(issue, withAttachmentRefs(body, issue.id, await writeAttachments(workspace, issue.id, input?.attachments)), author);
  await writeIssue(issue, record.metadata, pathname, workspace);
  return issue;
}
