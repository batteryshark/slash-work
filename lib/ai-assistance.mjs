...
async function readStoredConfig(pathname = aiConfigPath()) {
  try {
    ...
    if (parsed?.version !== CONFIG_VERSION) throw new Error("Unsupported version");
    return parsed;
  } catch (error) {
...
export async function getAiSettings(pathname = aiConfigPath()) {
  return publicConfig(await readStoredConfig(pathname));
}

export async function saveAiSettings(input, pathname = aiConfigPath()) {
  const current = await readStoredConfig(pathname);
...
function endpointFor(config) {
  if (config.provider === "anthropic-compatible") return config.baseUrl.endsWith("/messages") ? config.baseUrl : `${config.baseUrl}/messages`;
  return config.baseUrl.endsWith("/chat/completions") ? config.baseUrl : `${config.baseUrl}/chat/completions`;
}

async function modelRequest(config, messages, fetchImpl = fetch, timeoutMs = REQUEST_TIMEOUT_MS) {
  ...
  try {
    ...
    response = await fetchImpl(endpointFor(config), {
      method: "POST",
      headers: anthropic
...
export async function testAiSettings(pathname = aiConfigPath(), fetchImpl = fetch, timeoutMs = REQUEST_TIMEOUT_MS) {
  const config = await readStoredConfig(pathname);
  const result = await modelRequest(config, [
...
export async function createAiProposal(workspace, input, { configPath = aiConfigPath(), fetchImpl = fetch, timeoutMs = REQUEST_TIMEOUT_MS } = {}) {
