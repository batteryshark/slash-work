...
const operations = [
  ...
  },
  {
    ...
    category: "ideas",
    artifactType: "idea",
    summary: "List ideas preserved for later evaluation.",
    mutation: false,
    transport: { cli: "work ideas", api: { method: "GET", path: "/api/ideas" } },
    rules: ["An idea is a possibility, not a request.", "Do not turn an idea into implementation without a separate authorized task."],
  },
  {
    id: "ideas.create",
    category: "ideas",
    ...
    mutation: true,
    transport: { cli: "work idea \"title\" [--detail opportunity] [--project exact/path | --unassigned]", api: { method: "POST", path: "/api/ideas" } },
    inputSchema: object({
    ...
    rules: ["An idea never authorizes implementation.", "The local CLI uses the exact marked current project unless --unassigned is explicit.", "Leave unknown sections empty rather than inventing content."],
    example: { title: "Add a weekly review view", opportunity: "See what moved across projects each week.", projectPath: "software/rekit" },
  },
  {
    id: "ideas.update",
    category: "ideas",
    artifactType: "idea",
    summary: "Update an idea or transition its state.",
    mutation: true,
    ...
    inputSchema: object({
      ...
      status: { enum: ["open", "exploring", "deferred", "proposed", "adopted", "declined"] }, reason: string("Required when deferring or declining."),
  ...
  },
  {
    ...
    rules: ["The issue starts queued with an empty agents list; the human decides whether it becomes work.", "Filing an issue grants no authority to implement a fix.", "Agents cannot set or change the agents delegation list; only a human delegates work."],
  ...
  },
  {
    ...
    rules: ["Create executable work only when action is authorized.", "For local CLI work, run work agent context and keep its exact current project unless the user explicitly asks for --unassigned or another discovered project.", "For API work, use projects.list before assigning projectPath.", "Write observable acceptance criteria.", "Do not mark dependencies complete implicitly."],
    example: { title: "Implement agent capability discovery", type: "feature", priority: "high", projectPath: "software/rekit", goal: "Let fresh agents discover scoped Work instructions.", acceptanceCriteria: ["CLI prints the resolved current project", "API serves the same catalog"] },
  },
  {
    id: "tasks.update",
    category: "tasks",
...
export function getArtifactSchema(type = null) {
  ...
  if (!Object.hasOwn(ARTIFACT_SCHEMA.$defs, type) || !["capture", "note", "idea", "issue", "decision", "task"].includes(type)) return null;
  return {
