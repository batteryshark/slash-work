...
const operations = [
  {
    ...
    rules: ["Use only paths returned by this operation for projectPath.", "Do not infer project ownership from titles or body text."],
  },
  {
    id: "captures.list",
    category: "captures",
  ...
  },
  {
    ...
    category: "notes",
    artifactType: "note",
    summary: "Create durable reference material, optionally requesting agent review.",
    mutation: true,
    transport: { api: { method: "POST", path: "/api/notes" } },
    inputSchema: object({
      title: string("Short note title.", { maxLength: 300 }),
      text: string("Plain-text note content."),
      scopePath,
      projectPath,
      agentIntent: { enum: ["reference_only", "review_requested"], default: "reference_only" },
    }),
    rules: ["Default to reference_only.", "review_requested asks for review but never authorizes implementation."],
  ...
  },
  {
    ...
    category: "notes",
    artifactType: "note",
    summary: "Update note text, title, or review intent without replacing unspecified fields.",
    mutation: true,
    transport: { api: { method: "PATCH", path: "/api/notes/{id}" } },
    ...
    rules: ["Send only fields that should change.", "Do not treat review_requested as authorization to modify the project."],
  ...
  },
  {
    ...
    category: "notes",
    artifactType: "note",
    summary: "Delete a note the user no longer wants.",
    mutation: true,
    destructive: true,
    transport: { api: { method: "DELETE", path: "/api/notes/{id}" } },
    parameters: { id: recordId("note") },
    rules: ["Delete only with explicit user authorization."],
  },
  {
  ...
  },
  {
    ...
    rules: ["An open decision records a pending choice, not approval.", "Do not resolve a decision without the human's explicit choice."],
  },
  {
    id: "decisions.create",
    category: "decisions",
    ...
    inputSchema: object({ title: string("One-line decision question.", { minLength: 1 }), detail: string("Decision context."), projectPath, options: stringList("Explicit choices.") }, ["title"]),
    rules: ["Creating a decision does not imply approval.", "Keep it unassigned unless the project is explicit."],
    example: { title: "Where should the lab live?", options: ["Keep unassigned", "Assign later"], projectPath: null },
  },
  {
    ...
    rules: ["Record only an action the human explicitly chose.", "Do not translate discussion or an open card into approval."],
  ...
  },
  {
    ...
    rules: ["Do not move a task to done while dependencies are unfinished.", "Include a note when the reason is not obvious."],
  ...
  },
  {
    ...
    rules: ["Read the current task to obtain the checklist index.", "Change only the item whose result has been verified."],
...
export function listAgentOperations({ serviceVersion } = {}) {
  return versioned({
    operations: operations.map(({ inputSchema, example, rules, parameters, ...operation }) => ({
      ...clone(operation),
...
export function getAgentOperation(id, { serviceVersion } = {}) {
  ...
  return versioned({ operation: clone(operation) }, serviceVersion);
}
...
export function renderAgentOperationMarkdown(id, { serviceVersion } = {}) {
  ...
  const transport = [operation.transport.cli ? `- CLI: \`${operation.transport.cli}\`` : null, operation.transport.api ? `- API: \`${operation.transport.api.method} ${operation.transport.api.path}\`` : null].filter(Boolean).join("\n");
