...
const operations = [
  ...
  },
  {
    ...
    rules: ["Read the complete conversation before claiming or replying.", "The issue remains investigation-only authority even when it describes a desired code change.", "The payload's attachments array resolves every pasted image to {name, path} — path is the absolute file on this machine. Open it directly when local, or GET /api/attachments?record=<record-id>&name=<name> when remote. In the agent payload the issue body, reply bodies, and resolution summary have their ../attachments/ refs rewritten to those absolute paths; the record on disk keeps them relative to the record file in its .work directory."],
  },
  {
    id: "issues.claim",
    category: "issues",
  ...
  },
  {
    ...
    category: "issues",
    artifactType: "issue",
    summary: "Mark a claimed issue in progress, needing human input, or closed with a resolution summary.",
    mutation: true,
    transport: { api: { method: "POST", path: "/api/agent/issues/{id}/state" } },
    headers: agentHeaders,
    parameters: { id: issueId },
    inputSchema: object({
      state: { enum: ["in_progress", "needs_human", "closed"] },
    ...
    rules: ["Only the claiming agent can change agent-managed state.", "Include resolutionSummary when state is closed.", "Agents cannot reopen, delete, archive, or lock issues; closing is an agent opinion the human can always reopen."],
    example: { state: "closed", resolutionSummary: "The issue was caused by stale cached state; restarting refreshes it. No project changes were made." },
  },
  {
    id: "decisions.list",
    category: "decisions",
  ...
  },
  {
    ...
    inputSchema: object({ title: string("One-line decision question.", { minLength: 1 }), detail: string("Decision context."), projectPath, options: stringList("Explicit choices."), recommendedOption: nullableString("Recommendation. When set, it must exactly match one recorded option."), recommendationReason: string("Required from agents: with a recommendation, why that option; without one, why no lean is possible. The human decides on this reason, not on trust.", { maxLength: 2000 }), refs: stringList("Ids of the Work items this decision is about (for example a task id such as W-0001).") }, ["title", "recommendationReason"]),
    rules: ["Creating a decision does not imply approval.", "Recommend an option by default; a recommendation must exactly match one recorded option and never preselects it for the human.", "recommendationReason is required either way: with a recommendation it says why that option, without one it says why no lean is possible. A bare recommendation or a bare shrug are both rejected — only silence is refused.", "The local CLI treats the exact marked current project as explicit context; use --unassigned only for a workspace-level decision."],
    example: { title: "Which release strategy should this project use?", options: ["Canary", "All at once"], recommendedOption: "Canary", recommendationReason: "A staged rollout catches the config drift this repo has hit twice; all-at-once has no rollback story here.", projectPath: "software/rekit" },
  },
  {
  ...
  },
  {
    ...
    rules: ["Do not move a task to done while dependencies are unfinished.", "Account for every requirement and acceptance criterion before moving to review OR to blocked: tick each one you verified with `work check <id> requirement|acceptance <index>` (or tasks.checklist), and decline each one you did not with `--decline \"<reason>\"`. Both moves are refused with review_checklist_incomplete while any item is unaccounted for.", "Declining is always allowed and never a failure — being blocked on the first criterion is a valid reason to decline the rest. What is refused is leaving them unanswered.", "Include a note when the reason is not obvious."],
  ...
  },
  {
    ...
    category: "tasks",
    artifactType: "task",
    summary: "Check, reopen, or decline one requirement or acceptance criterion.",
    mutation: true,
    transport: { cli: "work check <id> requirement|acceptance <index> [--off|--decline \"why\"]", api: { method: "POST", path: "/api/tasks/{id}/checklist" } },
    parameters: { id: taskId },
    inputSchema: object({ section: { enum: ["requirements", "acceptance"] }, index: { type: "integer", minimum: 0 }, checked: { type: "boolean" }, declined: { type: "boolean", description: "Verified as not done. Requires reason." }, reason: string("Why this item was not done. Required with declined.") }, ["section", "index"]),
    rules: ["Read the current task to obtain the checklist index.", "Tick only an item whose result has been verified.", "Decline an item you did not do, with a reason: not attempted, no longer applicable, or blocked on something named.", "Every requirement and acceptance criterion must be ticked or declined before a task moves to review or to blocked."],
