...
const operations = [
  ...
  },
  {
    ...
    rules: ["Do not move a task to done while dependencies are unfinished.", "Verify and check every requirement and acceptance criterion before moving to review: tick each one with `work check <id> requirement|acceptance <index>` (or tasks.checklist) as you verify it, or the move is refused with review_checklist_incomplete.", "Include a note when the reason is not obvious."],
  ...
  },
  {
    ...
    mutation: true,
    transport: { cli: "work check <id> requirement|acceptance <index> [--off]", api: { method: "POST", path: "/api/tasks/{id}/checklist" } },
    parameters: { id: taskId },
